    ...
    >
      ...
      <Pressable onPress={() => openTrip("guro")} style={s.featureTrip}>
        ...
        <View style={s.featureText}>
          ...
          <Text style={s.featureTitle}>전주 한옥마을</Text>
          <Text style={s.featureSub}>느긋한 숙소와 버섯전골</Text>
...
function Search({ openTrip }: { openTrip: (id: string) => void }) {
  ...
  const items = useMemo(
    () =>
      [
        { title: "소나기식당", type: "장소", trip: "전주 한옥마을", id: "guro" },
        {
          title: "버섯전골",
          type: "레시피",
          trip: "전주 한옥마을",
...
function Places() {
  return (
    <View>
      ...
      <Place name="소나기식당" meta="8월 22일 · 디너 예약" tag="RESERVED" />
      <ContentHead no="02" title="가보고 싶은 곳" note="SAVED" />
      <Place name="소나기식당 본점" meta="전주 한옥마을 · 스시" />
      <Place name="온기식탁 완산점" meta="8월 21일 · 점심" />
      <Place name="바다초밥 가산점" meta="다음에 가요" />
    </View>
  );
}
function Food() {
  return (
    <View>
      <View style={s.recipeBlock}>
        ...
        <Text style={s.recipeName}>버섯전골</Text>
  ...
  return (
    <View>
      ...
      <Note
        ...
        text="소나기식당은 일요일 13:30으로 생각하고 있어요."
      />
      <Note
        who="동행"
        when="YESTERDAY, 21:06"
        text="달빛한옥 체크인 15시, 체크아웃 12시 확인했어."
      />
    </View>
  );
}
