...
// 라이선스가 요구하는 저작권 안내와 전문 링크는 우리 탭 > 앱과 계정 > 오픈소스
// 라이선스에 있다. WarmAppShell.tsx의 openSourceNotices를 보라(규칙 9장).
...
/**
 * 크기 스케일. 스타일시트의 모든 fontSize가 이 단계 중 하나다.
 * 역할마다 쓰는 단계가 다르다. 같은 제목이라도 화면 제목과 목록 행의 제목은
 * 단계가 달라야 하므로 역할 하나에 크기 하나를 강제하지 않는다.
 *
 * 13과 15는 한 줄에 여러 값이 들어가는 빽빽한 줄을 위해 둔다. 지출 표나 예약
 * 줄처럼 이름과 숫자가 나란히 놓이는 자리는 14로 올리면 넘치고 12로 내리면
 * 읽기 힘들다. 32는 비용 탭의 합계 하나에만 쓴다.
 *
 * 아래 하한은 지켜야 한다. 읽어야 하는 글자를 11 밑으로 내리지 않는다.
 */
export const sizes = [11, 12, 13, 14, 15, 16, 18, 20, 24, 28, 32, 34] as const;
