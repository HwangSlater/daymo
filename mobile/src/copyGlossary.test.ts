...
const 금지: [RegExp, string][] = [
  ...
  [/지우기|지우지|지우고|지울까요|지웠어요|지우는 중|지워요(?![가-힣])/, "삭제"],
  [/홈 화면에|홈에 쓰/, "대표 사진"],
  [/이용자/, "타는 사람 / 멤버"],
  ...
  [/프로토타입/, "버전만"],
  [/\d+개의 /, "「여행 N개」처럼 수를 뒤에"],
  [/(적어|남겨|만들어|골라|넣어|묶어|꾸려|펼쳐|채워|담아|해)(보세요|볼까요|봐요|보는|볼게요)/, "보조용언은 띄어 쓴다 (적어 보세요)"],
  [/(?<![주보두])세요/, "~해요 / ~해 주세요 (「~하세요」는 쓰지 않는다)"],
  [/[가-힣]셨/, "상대 높임은 쓰지 않는다 (「받았어요」)"],
  [/[가-힣] ?(드려요|드릴게요|드립니다|드릴까요)/, "「~해요」 (낮춰 모시는 말투를 쓰지 않는다)"],
  [/저희/, "「우리」 또는 주어 없이"],
  [/받는 중|받아 오는 중/, "불러오는 중"],
  [/쓴 돈/, "지출 / 총 지출"],
  [/[가-힣] ?고치기|고쳐요|고치러/, "수정"],
  [/편집(?! 가능)/, "수정 (권한 값 「편집 가능」만 그대로)"],
  [/카톡/, "카카오톡"],
  [/붙여 넣/, "붙여넣기 (한 낱말로 붙여 쓴다)"],
  [/잠시 뒤/, "잠시 후"],
  [/이 날(?![가-힣])/, "이날"],
  [/다시하기/, "다시 하기"],
  [/\d+일간/, "N일 동안"],
  [/\}(을|를|이|가|은|는)(?![가-힣])/, "josa() 또는 조사 없는 문장"],
];
