...
성능 예산은 기능 축소 허가가 아니다. 목표를 맞추기 위해 기능 제거, 결과 생략, 사진 화질·원본 정책 저하, 오프라인 범위 축소, 보존 기간 단축, 자동 갱신 제거 또는 사용자 단계 추가가 필요하다고 판단되면 구현 전에 현재 측정값·병목 근거·영향 기능·최소 2개 대안·되돌리기 방법을 사용자에게 제시하고 승인을 받는다. 캐시·쿼리·인덱스·렌더링 최적화처럼 사용자 계약을 바꾸지 않는 개선은 별도 승인 없이 수행한다.
...
- 사진 캐시 사용량 표시·사용자 비우기·OS purge 복구와 Wi-Fi/데이터 절약 정책
...
11. 오래된 cursor 만료→bootstrap 복구, 로그아웃→계정 cache 잠금, 같은 계정 재로그인 복원, 다른 계정 격리
...
- 공동 데이터 SQLite는 OS sandbox의 계정별 namespace에 두고 명시적 로그아웃 시 잠금·공간 권한 상실 또는 계정 최종 삭제 시 제거
- 사진 cache는 앱 자체 용량·기간 상한을 두지 않되 설정에서 현재 사용량과 `캐시 비우기`를 제공하고 OS가 cache를 지워도 재다운로드로 복구
...
- 인증 메일 재전송 60초 간격·계정/IP별 일 5회 제한과 `Retry-After` 테스트
- 로그인 15분 내 5회 실패 이후 점진적 지연·최대 15분 일시 제한, 성공 초기화와 IP 제한 분리 테스트
- 가입·로그인·인증·비밀번호 재설정 응답의 계정 존재 여부·OAuth 방식 노출 차단 테스트
- 정상 가입에는 CAPTCHA가 없고 위험 신호가 있을 때만 challenge가 필요한지 테스트
...
- 원본은 private storage에 보존하고 표시본·썸네일에서는 EXIF GPS·기기 정보를 제거하며 촬영일만 별도 저장
- HEIC/HEIF 원본 보존, 1440px JPEG 표시본·480px JPEG 썸네일의 방향·색상·무확대 변환 테스트
- 같은 여행 checksum 중복 후보 안내와 `그래도 추가` 경로 테스트
...
- [ ] 데이터 손실·권한 노출·로그인 불가·반복 crash 출시 차단 오류 0건
- [ ] 최근 7일 crash-free session 99.5% 이상, API 전체 p95 1초 이하
- [ ] 사용자가 iOS·Android 실기기 smoke, 전체 백업 복원 결과와 store 제출본을 최종 승인

베타 최소 기간은 두지 않는다. 위 품질 gate를 모두 통과하기 전에는 기간과 관계없이 출시하지 않고, 모두 충족하면 임의의 2주 대기 없이 출시를 검토한다. CI와 자동 server 배포는 store 제출·공개 승인을 대신하지 않는다.

출시 gate의 API p95 1초는 허용 상한이며 1절의 일반 조회 500ms·쓰기 800ms 내부 목표를 완화하지 않는다.
