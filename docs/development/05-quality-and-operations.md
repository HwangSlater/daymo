...
상태 감시는 UptimeRobot 무료 plan의 외부 5분 확인으로 시작한다. 초기 상태 페이지는 UptimeRobot 제공 URL을 사용해 VPS 전체 장애 중에도 열리게 한다. 무료 plan에는 custom domain이 포함되지 않으므로 `status.daymo.xyz`는 유료 전환 또는 별도 외부 host 도입 전까지 사용하지 않는다. 사용자는 현재 장애와 복구 여부만 확인하며 최근 90일 같은 공개 incident history는 제공하지 않는다. 내부 운영 기록은 별도 보존 정책에 따라 유지한다.
