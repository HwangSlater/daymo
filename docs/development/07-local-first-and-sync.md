...
**여기까지는 목표이고, 2026-09-23 현재 구현된 것과 다르다.** 지금 앱은 SQLite 대신 AsyncStorage 한 열쇠(`daymo.trip-data.v1`)에 모든 여행을 통째로 넣고, ETag/304·sync cursor 가 없다. 여행을 열면 탭과 상관없이 목록 열두 개를 받는다. 고칠 차례와 까닭은 2026-09-23 검토 보고서(묶음 H·J)에 있다.

### 사진 파일 캐시 (2026-09-23 구현)

- 폴더는 문서 폴더의 `trip-photos/` 하나다. 캐시 폴더로 옮기지 않았다 — OS 가 아무 때나 비우는데 기기에 적어 둔 사진 자리는 그대로 남아 빈 칸이 된다.
- 한 폴더에 **다시 받을 수 있는 것**(표시본·썸네일·저장하려고 잠깐 받은 원본, 이름이 `server-` 로 시작)과 **여기밖에 없는 것**(기기에서 고른 원본)이 섞여 있다. 이름으로 가른다(`photoCachePlan.cacheKindOf`). 고른 원본은 아직 못 올린 사진의 하나뿐인 원본일 수 있어 지우지 않고 용량으로도 세지 않는다.
- 상한은 **200MB**다(`photoCachePlan.PHOTO_CACHE_LIMIT`). 아래 500MB 권고보다 작게 잡은 까닭: 표시본 한 장이 0.5MB 안팎이라 200MB 면 400장이고, 이 폴더는 문서 폴더라 iOS 에서 iCloud 백업에 들어간다. 지워도 다시 받으면 그만이다.
- 쓰는 곳이 없는 것(옛 판 표시본, 저장이 끊겨 남은 원본 사본)은 상한과 상관없이 지운다. 그래도 넘치면 **오래전에 받은 것부터** 지운다(`planPhotoSweep`). 사진을 받은 뒤 3분에 한 번꼴로 돈다(`maybeSweepPhotoCache`).
- 용량 재기·비우기는 `photoCache.photoCacheUsage()` · `clearPhotoCache()` 다. 설정 화면에 「사진 캐시 N · 비우기」로 붙인다(`cacheSizeText`).
- 지워진 파일 자리를 화면이 그대로 쓰지 않게 `isLivePhotoUri` 가 폰에서는 파일이 있는지도 본다(자리마다 한 번만 묻고 기억한다).

초기 권장 파일 캐시 상한은 500MB다(위와 같이 200MB 로 좁혔다). 사진 업로드 네트워크 기본값은 `Wi-Fi 및 모바일 데이터`이고, 사용자가 기기 설정을 `Wi-Fi에서만 업로드`로 바꾸면 모바일 데이터에서 선택한 원본을 영속 대기열에 보관한다. Wi-Fi 연결과 OS 실행 기회가 생기면 추가 조작 없이 자동 재개하며 앱 재실행 후에도 대기열을 복원한다. 사용자는 대기 상태에서 `모바일 데이터로 지금 업로드`를 선택할 수 있다. 데이터 절약 모드의 캐시 상한은 150MB와 낮은 품질 썸네일을 제안하되, 수치는 실제 파일럿 측정 후 조정한다.
