...
**여기까지는 목표이고, 2026-09-23 현재 구현된 것과 다르다.** 지금 앱은 SQLite 대신 AsyncStorage 한 열쇠(`daymo.trip-data.v1`)에 모든 여행을 통째로 넣고, ETag/304·sync cursor·파일 캐시 LRU·용량 상한·「캐시 비우기」가 없다. 여행을 열면 탭과 상관없이 목록 열두 개를 받고, 남이 올린 사진의 표시본을 모두 받아 문서 폴더에 영구 보관한다. 고칠 차례와 까닭은 2026-09-23 검토 보고서(묶음 H·J)에 있다.
