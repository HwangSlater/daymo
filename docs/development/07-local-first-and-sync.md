...
- 데이터 절약 모드, 원본 업로드를 Wi-Fi까지 대기할지 설정
...
초기 권장 파일 캐시 상한은 500MB다. 사진 업로드 네트워크 기본값은 `Wi-Fi 및 모바일 데이터`이고, 사용자가 기기 설정을 `Wi-Fi에서만 업로드`로 바꾸면 모바일 데이터에서 선택한 원본을 영속 대기열에 보관한다. Wi-Fi 연결과 OS 실행 기회가 생기면 추가 조작 없이 자동 재개하며 앱 재실행 후에도 대기열을 복원한다. 사용자는 대기 상태에서 `모바일 데이터로 지금 업로드`를 선택할 수 있다. 데이터 절약 모드의 캐시 상한은 150MB와 낮은 품질 썸네일을 제안하되, 수치는 실제 파일럿 측정 후 조정한다.
...
- 데이터 절약 모드에서 사진 원본이 자동 다운로드되지 않고, 사용자가 선택한 업로드는 예상 용량 확인 또는 Wi-Fi 대기 후 수행
