...
| ID | 결정 | 권장안 | 이유 |
| --- | --- | --- | --- |
| D-001 | 출시 앱 식별자 | 소유 도메인 기반 ID 확정 | OAuth, Universal Link와 스토어 서명에 영향 |
| D-002 | Node 기준 | 당시 Expo 지원 LTS 하나로 고정 | 현재 v26은 팀 재현성이 낮음 |
| D-003 | Expo SDK | 별도 브랜치에서 최신 안정판 업그레이드 검증 후 채택 여부 결정 | 현재 audit 경고와 장기 지원성 |
| D-004 | 초기 가입 범위 | 초대 기반 비공개 알파 | 개인정보·운영 부담을 낮추고 공동 사용 검증 가능 |
...
| D-006 | 타인 메모 삭제 | owner만 가능, editor는 본인 메모만 |
| D-007 | 가입 연령 | MVP는 만 14세 이상 |
| D-008 | 마지막 owner 탈퇴 | 소유권 이전 후에만 허용 |

### 여행·콘텐츠 전

| ID | 결정 | 권장안 |
| --- | --- | --- |
| D-009 | 요리 탭 원본 | `cookingEnabled`를 여행 설정으로 저장 |
| D-010 | 재료↔준비물 완료 | 상태 분리, 연결과 반영 제안만 제공 |
...
| D-013B | 사진 외부 백업 | Google Drive에 restic 암호화 snapshot 자동 백업, 사용할 계정은 결정 필요 |
...
1. 실제 앱 식별자에 사용할 소유 도메인 또는 역도메인 문자열
2. Node/Expo SDK 기준을 현재 조합 유지와 선행 업그레이드 중 무엇으로 할지
3. 첫 알파를 초대 전용으로 운영할지
4. Google Drive 백업용 계정을 개인 계정으로 시작할지 Daymo 전용 계정으로 분리할지
