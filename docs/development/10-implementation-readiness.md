...
| D-003 | Expo SDK | **결정 완료: 기능 개발 전 최신 안정판 업그레이드 검증** | 검증 커밋에서 SDK 54를 57로 올렸고 되돌리지 않음 |
| D-004 | 초기 가입 범위 | **결정 완료: 처음부터 공개 회원가입** | 여행 공간과 콘텐츠는 계속 초대 멤버에게만 비공개 |
| D-004A | 저장소 라이선스와 공개 여부 | **미결정. 백엔드 착수 때 정한다** | 지금은 LICENSE 파일이 없어 기본값대로 모든 권리 유보다. 상용 앱 소스에는 그 상태가 맞고, 오픈소스 라이선스를 붙이면 누구나 같은 앱을 만들어 팔 수 있다. 저장소를 비공개로 돌릴지도 함께 정한다. 과거 히스토리 재작성으로 GitHub 서버에 남은 옛 객체를 확실히 없애려면 비공개 전환이나 GitHub 지원 요청이 필요하다 |
...
| D-015 | 사진 한도 | **결정 완료: 사진당 20MB, 공간당 1GB, 전체 10GB, 초기 동영상 미지원** | 초기에는 50GB 기본 디스크의 `/srv/daymo/uploads`를 쓰고, 증가 시 미니PC·NAS로 이전한다 |
...
| D-016A | VPS 처리 국가 | **2026-09-14 iwinv 한국 리전 서버 생성**, 계약·관리 화면에서 실제 데이터센터 국가/세부 지역 최종 확인 필요 |
...
| D-025A | 사진 variant | **결정 완료: 원본 보존 + 2048px 표시본 + 480px 썸네일** |
...
| D-027 | 베타 기간 | **결정 완료: 최소 기간 없음**, 품질 gate 충족 여부로 종료 판단 |
| D-027A | 출시 차단 오류 | **결정 완료: 데이터 손실·권한 노출·로그인 불가·반복 crash 0건** |
| D-027B | 안정성 기준 | **결정 완료: 최근 7일 crash-free session 99.5% 이상·API p95 1초 이하** |
| D-027C | store 출시 | **결정 완료: 체크리스트·실기기·백업 복원 후 사용자 최종 승인**, CI 자동 출시 금지 |
| D-027D | 성능 trade-off | **결정 완료: 기능·품질·오프라인·보존·사용 흐름 저하 전 사용자 검토·승인 필수** |
...
개발 전 제품·보안·인프라 정책 결정은 D-001~D-027D로 완료했다. 이후에는 정해진 문서 범위에서 구현하며 사소한 라이브러리·내부 수치는 별도 승인 없이 검증 가능한 기본값을 사용한다.

다음 경우에는 구현 전에 사용자 승인을 다시 받는다.

1. 기능·화질·오프라인 범위·데이터 보존·사용 흐름을 낮추는 성능 trade-off
2. 유료 서비스 도입 또는 VPS·Google Drive·Vercel·UptimeRobot 구성 변경
3. 개인정보 수집·외부 전송·보유기간 변경
4. 공개 API·DB 호환성을 깨는 변경
5. store 제출과 production 공개

성능 trade-off 검토에는 현재 측정값, 병목 근거, 영향 범위, 최소 2개 대안과 각 되돌리기 방법을 포함한다.
