...
원본은 HEIC/HEIF를 포함해 선택한 형식 그대로 private volume에 보존한다. 서버는 촬영일만 별도 저장하고 GPS·기기 EXIF를 제거한 긴 변 1440px JPEG 표시본과 480px JPEG 썸네일을 만든다. 같은 여행에서 checksum이 같은 사진은 중복 후보로 안내하지만 사용자가 원하면 추가할 수 있다.
