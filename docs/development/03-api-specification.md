...
| Method | Path | 용도 |
| --- | --- | --- |
| POST | `/client-errors` | 앱이 멈췄을 때 오류 한 줄 |

앱에는 오류 수집 SDK를 넣지 않는다. 대신 앱이 스스로 거른 한 줄을 여기로 보내고, 서버가 한 번 더 거른 뒤 로그에 남기거나 Sentry로 올린다([05-quality-and-operations.md](./05-quality-and-operations.md) 5장).

로그인하지 않아도 부를 수 있다. **로그인 화면에서 난 오류가 가장 알고 싶은 것이라 token을 요구하면 정작 필요한 것을 못 받는다.** 대신 누가 보냈는지는 적지 않고, 프로세스 하나가 1분에 받는 수를 60개로 막는다. 무엇을 받았든, 한도를 넘겨 버렸더라도 같은 `202`를 준다.
...
```json
{
  ...
  "kind": "crash",
  "name": "TypeError",
  "message": "undefined 를 읽을 수 없다",
  "where": "/app"
}
```

칸은 이 여섯 개뿐이고 모르는 칸이 오면 `422`다. 자유 입력 칸을 늘리면 언젠가 거기로 본문이 들어온다. `kind`는 `crash`(앱이 멈춤)·`unhandled`(처리되지 않은 promise)·`api`(서버 호출 실패)다. 스택은 받지 않는다. 앱은 같은 오류를 5분에 한 번, 한 시간에 열 번까지만 보낸다.
...
계정당 활성 기기 세션은 최대 5개다. 여섯 번째 기기 로그인 transaction에서 가장 오래 사용하지 않은 다른 기기의 refresh token family와 push token을 폐기한다. 로그인 응답의 `evictedSession`에는 민감정보 없이 기기 표시 이름과 마지막 사용 시각만 포함하고, 앱은 `오래 사용하지 않은 기기에서 로그아웃했어요` 안내를 보여준다. 사용자는 `/auth/sessions`에서 현재 기기를 구분해 다른 세션을 직접 종료할 수 있다. 앱의 **우리 → 내 프로필 → 로그인한 기기**가 이 목록을 그린다(2026-09-16). 기기 이름·마지막 사용 시각·`지금 이 기기` 표시와 해지 단추가 한 줄씩 있고, 4대부터는 한도가 가깝다는 안내를 함께 보여 준다. 끊긴 뒤에 한 번 알리는 것만으로는 웹(사파리)·웹(크롬)·폰을 오가는 사람이 왜 로그아웃됐는지 알 수 없어서다. 지금 이 기기를 해지하면 로그아웃과 같으므로 앱이 다르게 묻고, 성공하면 바로 로그인 화면으로 돌아간다.
...
| GET | `/trips/{tripId}/photos` | 사진 목록(`targetType`·`targetId`·`date`로 거르기) |
| PATCH/DELETE | `/photos/{photoId}` | 설명·날짜·붙은 곳 수정, 삭제 |
...
- `settings`는 카드에서 꾸민 것 한 덩어리다: `{style(없음|필름|엽서|스크랩북|네컷|네컷 격자|네컷 가로|세컷), ratio(세로|정사각|가로), photoIds(최대 4장), title, caption, parts, stats, frameColor(검정|흰색|크림|노을|바다|숲), stickers(옛 칸), decor(손으로 얹은 스티커·글자, 30개까지), dateStamp, photoCaptions}`(괄호 안은 지금 앱이 쓰는 값이다. 서버는 이 목록으로 막지 않는다 — 아래 모르는 값 규칙). 값은 앱 화면에 보이는 말 그대로다(한국어만 쓰는 앱이라 영어 코드를 따로 두지 않는다). 뒤의 넷은 네컷 틀에서만 그려지고 다른 스타일에서는 저장만 된다. 서버는 이 값으로 아무것도 계산하지 않고 그대로 돌려주며, `photoIds`가 그 여행의 다 올라온 사진인지만 본다(아니면 `422`). 칼럼을 여럿 두지 않고 JSONB 한 칸인 이유는 카드에 무엇을 넣고 뺄지가 화면을 고칠 때마다 바뀌는 값이어서다.
- 완성 이미지(2026-09-22): 앱이 카드를 「완료」할 때 원본 화질로 그린 그림을 `PUT /trip-cards/{cardId}/image?version=N` 으로 올린다. 본문은 파일 그대로(multipart 아님), PNG·JPEG 20MB·1600만 픽셀까지다. 서버는 긴 변을 줄이지 않고 품질 92 JPEG 로 다시 써 `trips/{tripId}/cards/{cardId}-v{N}.jpg` 에 두고, 크기는 공간 저장 한도에 사진과 함께 센다. `version` 이 지금 카드 버전과 다르면 `409`(옛 그림), 카드를 고칠 수 없는 사람은 `403`이다. 올려도 카드 `version` 은 오르지 않는다. `GET /trip-cards/{cardId}/image` 는 공간 멤버에게 JPEG 를 주고 없으면 `404`다. 카드 응답의 `imageVersion` 이 `version` 과 같을 때만 그 그림이 지금 카드와 같다 — 앱은 그때 받아 쓰고 아니면 그 자리에서 그린다. 까닭은 사진 원본이 30일 뒤 지워져, 그 뒤에는 기기가 원본 화질로 다시 그릴 수 없어서다. 카드를 지우면 파일도 지우고, 여행·공간이 지워지면 여행 폴더째 사라진다.
- **모르는 값 규칙(2026-09-22).** 새 스티커·칸·값이 더해져도 앞뒤 판이 서로의 것을 지우지 않게 한다.
  - 서버는 `settings` 의 값을 목록으로 막지 않는다. 틀 이름·스티커 이름(`decor[].kind`)·`parts`·`stats`·`stickers` 는 1~20자 글자면 받고, 모르는 칸도 `settings` 바로 아래와 `decor` 줄 안에서 받은 이름 그대로 저장해 돌려준다. 새 스티커를 더할 때 서버를 먼저 올릴 일이 없다. 막는 것은 크기다: `settings` 전체가 16KB(`KEEPSAKE_SETTINGS_MAX_BYTES`)를 넘으면 `422`, 스티커 30개·사진 4장·제목 60자·글자 24자, 자리 값의 범위도 그대로 본다.
  - 앱은 모르는 값을 만나면 기본으로 그리되 버리지 않는다(`mobile/src/tripCard.ts` 의 `KeepsakeKept`, `cardDecor.ts` 의 `extra`·`unknownDecorOf`). 모르는 스티커 줄과 칸은 받은 그대로 되돌려 보내고, 모르는 값이라 기본으로 그린 칸(틀·비율·틀 색)은 사람이 바꾸지 않았으면 원래 값을 보낸다. 이렇게 하는 앱은 카드를 만들고 고칠 때 `X-Daymo-Card-Keeps-Unknown: 1` 을 붙인다.
  - 이 표시가 없는 `PATCH /trip-cards/{cardId}` 는 모르는 값을 버리는 옛 앱(1.0.0, 빌드 3 — 이미 심사에 들어가 고칠 수 없다)이 보낸 것으로 본다. 서버가 지금 저장된 값에서 그 앱이 버렸을 것을 되살려 합친다: 모르는 이름의 스티커 줄(이름이 겹치면 새 이름을 준다), 모르는 칸, 아는 스티커 줄에 붙어 있던 모르는 칸, 옛 앱의 기본값(`필름`·`세로`·`검정`)으로 온 칸의 모르는 값, `parts`·`stats` 에 섞인 모르는 값. 옛 앱이 아는 것의 목록은 `services/trip_cards.OLD_APP_*` 에 얼려 두었고 **고치지 않는다**.
  - 호환 시험: `mobile/src/tripCard.test.ts`(읽고 다른 것을 고쳐 저장해도 모르는 값이 남는다), `backend/tests/test_api_trip_cards.py`(새 앱이 보낸 것을 그대로 돌려주고, 옛 앱의 수정이 지우지 않는다).
- 예전에는 `trips.card_settings`(JSONB) 한 칸이라 새로 만들면 앞서 만든 카드가 조용히 덮였다. 마이그레이션 `a2f9c6d08b14`가 그 값을 여행마다 첫 카드 한 줄로 옮기고 칸을 지웠다.
- `PATCH /trips/{tripId}`의 `coverPhotoId`는 홈 화면의 여행 카드에 깔 사진 한 장이다(`trips.cover_photo_id`, 사진을 지우면 `SET NULL`). 그 여행의 다 올라온 사진만 받고(아니면 `422`) `null`이면 해제다. 홈은 이 사진의 썸네일(`variant=thumbnail`)만 받아 카드에 깔고, 못 받으면 예전 종이 카드 그대로 그린다.
- 같은 `PATCH`의 `coverCardId`는 기념 카드 한 장을 통째로 깐다(`trips.cover_card_id`, 카드를 지우면 `SET NULL`, 마이그레이션 `b6e1f70c9d25`). 그 여행의 카드만 받고(아니면 `422`) `null`이면 해제다. **홈 카드는 여행마다 하나다** — 사진을 고르면 서버가 카드 쪽을 풀고, 카드를 고르면 사진 쪽을 푼다. 둘을 한 번에 보내면 `422`다.
- 여행 응답의 `coverPhotoIds`는 홈이 그릴 사진을 고른 차례대로 담고(카드를 깔았으면 그 카드의 `photoIds`, 사진 한 장이면 그 한 장, 아무것도 없으면 빈 목록), `coverCardStyle`은 그 카드의 틀 이름이다. 서버는 고를 때 사진을 베껴 두지 않고 볼 때마다 카드에서 읽는다. 그래야 카드를 고쳐 사진을 바꾸면 홈도 함께 바뀐다. 사진을 어떻게 놓을지는 앱이 이 둘로 정한다(카드 그림은 기기가 그린다, `mobile/src/tripCard.homeCoverRows`). 홈의 사진 자리는 가로로 넓어(약 1.6:1) 사진을 세로로 쌓는 틀(`네컷`·`세컷`)은 앱이 깔지 못하게 막는다. 눕히거나 격자로 바꿔 담으면 만든 사람이 고른 모양과 달라진다.
- `PATCH /trips/{tripId}`의 `coverFocusX`·`coverFocusY`(0~1)·`coverZoom`(1~4)은 **대표 사진에서 홈 카드 틀에 보여 줄 부분**이다(`trips.cover_focus_*`). `coverFocusX`·`coverFocusY`는 틀 한가운데에 놓을 점의 자리(사진에서의 비율, 왼쪽 위가 0,0)고, `coverZoom`은 틀을 꽉 채우는 가장 작은 크기를 1로 본 배수다. 기본은 0.5·0.5·1이고 그것이 지금까지처럼 가운데를 자른 모습이다. 홈 카드의 사진 틀은 가로로 넓어서(1.62:1) 세로 사진은 가운데 띠만 남는데, 어디를 남길지는 사람이 정하는 것이라 기기가 아니라 여행에 저장한다(같이 쓰는 사람의 홈에도 같게 보인다). **`coverPhotoId`나 `coverCardId`가 바뀌는데 이 값을 함께 보내지 않으면 서버가 기본값으로 되돌린다** — 다른 사진을 골랐는데 앞 사진의 자리가 남아 있으면 엉뚱한 데가 보인다. 셈은 앱의 `mobile/src/coverCrop.ts`가 한다.
...
앱은 선택 직후 예상 크기를 안내하지만 서버가 실제 byte와 quota를 최종 검증한다. 공간 사용량이 80%에 도달하면 사진 화면과 업로드 완료 화면에서 한 번 사전 경고하고, 100%에 도달하면 기존 사진의 조회·다운로드·삭제·복구는 유지한 채 새 업로드만 차단한다. 표시본과 썸네일은 자동으로 압축하거나 지우지 않는다.

**원본 보관은 30일이다**(2026-09-16 결정, `app.services.photos.ORIGINAL_DAYS`). 올린 지 30일이 지나면 정리 작업이 원본 파일만 지우고 `original_path`를 비운다. 사진 줄과 표시본·썸네일은 그대로 남아 여행 기록은 사라지지 않는다. 원본은 기록이 아니라 함께 간 사람이 받아 갈 파일이고(찍은 사람 폰에는 이미 있다), 화면은 표시본만 쓰기 때문이다. 계속 두면 아무도 열지 않는 파일이 공간 한도의 대부분을 차지한다.

사진 응답의 `originalUntil`이 원본을 받을 수 있는 마지막 때다. 원본이 이미 지워졌으면 `null`이고, 그때 `variant=original`은 410 `GONE`으로 답한다. 앱은 크게 보기 화면에 남은 기한을 한 줄로 보여주고, 기한이 지난 뒤 저장을 누르면 오류 대신 표시본을 저장한다.
