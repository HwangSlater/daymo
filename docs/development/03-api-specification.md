...
공간 삭제 요청은 owner 재인증과 공간 이름 재확인을 요구한다. 요청 성공 즉시 모든 멤버의 일반 공간 목록, sync 응답, 알림 대상에서 숨기고 7일 뒤 최종 삭제를 예약한다. 유예기간에는 owner의 삭제 예정 공간 화면에서만 확인·복구할 수 있다. 복구하면 기존 membership과 콘텐츠를 그대로 되살리고, 7일이 지나면 복구 API는 `410 GONE`을 반환한다.
