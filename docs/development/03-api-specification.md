...
생체 앱 잠금은 서버 API가 아니라 기기 설정이다. 기본값은 꺼짐이며 사용자가 켜면 앱 foreground 복귀 시 OS Local Authentication으로 화면을 잠근다. 생체정보 원본은 앱·서버가 수집하지 않고 OS 성공 여부만 사용한다. 실패·미등록·기기 변경 시 OS가 허용한 기기 자격 증명 또는 로그아웃을 제공하되 계정 삭제·로그인 방식 변경 등에는 기존 `reauthProof`를 별도로 요구한다.
