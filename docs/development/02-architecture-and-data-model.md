...
- OS 잠금화면 푸시는 여행명·멤버명·준비물명 등 공동 콘텐츠를 넣지 않고 일반 문구와 opaque notification ID만 사용한다. 생체 앱 잠금은 기기별 opt-in이며 기본값은 꺼짐이다. Face ID·지문 검증은 로컬 화면 잠금에만 사용하고 서버의 민감 작업 재인증을 대체하지 않는다.
