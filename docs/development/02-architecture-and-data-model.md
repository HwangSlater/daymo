...
- 관리자 웹은 마지막 조작 후 1시간이 지나면 잠그고 TOTP를 다시 확인해야 한다. 일반 사용자 자동 로그인 정책을 관리자 session에 적용하지 않는다.
- 신고 기록은 처리 완료 후 1년, 관리자 감사 로그는 생성 후 2년 보관하고 법적 보존 사유가 없으면 자동 파기한다. 신고 처리 결과는 14일간 이의를 받을 수 있다. 긴급 신고는 운영자에게 즉시 알리고 일반 신고는 1시간 단위로 묶어 알린다.
- 내 데이터 내보내기는 비동기 job으로 생성하며 계정·동의·공간 참여 이력, 본인 작성 콘텐츠와 본인이 올린 사진 원본만 포함한다. 다른 멤버가 작성하거나 올린 원본은 포함하지 않는다. archive와 1회용 download token은 준비 후 24시간에 만료·삭제한다.
- OS 잠금화면 푸시는 여행명·멤버명·준비물명 등 공동 콘텐츠를 넣지 않고 일반 문구와 opaque notification ID만 사용한다. 생체 앱 잠금은 기기별 opt-in이며 기본값은 꺼짐이다. 활성화한 기기는 백그라운드 진입 후 1분이 지나면 잠그고 Face ID·지문 또는 OS 기기 암호로 해제한다. 생체 검증은 로컬 화면 잠금에만 사용하고 서버의 민감 작업 재인증을 대체하지 않는다. 앱 전환 화면은 항상 privacy cover로 가리되 사용자의 스크린샷 촬영은 차단하지 않는다.
...
- 다른 멤버가 올린 사진에 등장한 사람이 삭제·처리정지를 요청하면 사진을 `restricted`로 전환해 일반 조회와 다운로드에서 임시 숨긴다. 운영 검토 중 업로더와 owner에게 요청자의 불필요한 개인정보 없이 사실과 절차만 알리고, 확인 결과에 따라 7일 삭제 절차로 전환하거나 `ready`로 복원한다.

## 5. 동시 편집과 오프라인

- 증분 sync API가 공동 데이터 정합성의 기준이다. foreground에서는 SSE로 변경 신호를 받고 해당 Query key 또는 sync를 갱신한다. 연결 실패·이벤트 유실은 앱 실행·foreground 복귀·네트워크 재연결·사용자 새로고침과 backoff polling fallback으로 복구하며 정상 상태에서 고정 짧은 주기 polling은 하지 않는다.
...
- 여행 상세은 탭별 지연 조회하되 상단 여행 정보와 첫 탭은 한 번에 받는다.
  - **아직 그렇지 않다(2026-09-23).** 지금은 여행을 열면 탭과 상관없이 목록 열두 개를 한꺼번에 맞춘다
    (`WarmTripDetail.tsx` 의 `useListSync` 12회). 사진·기록이 늘면 여는 데 걸리는 시간과 저장 횟수가 함께 늘어난다.
...
- 표시본은 긴 변 최대 2048px JPEG, 썸네일은 긴 변 최대 480px JPEG로 생성하며 작은 원본을 확대하지 않는다.
