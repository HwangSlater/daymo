...
VPS의 물리 위치가 대한민국 밖이면 계정, 여행, 메모, 사진의 국외 보관이 될 수 있다. 계약한 리전, 이전 국가, 이전받는 자, 목적, 항목, 시점·방법, 보유기간과 보호조치를 확정해 개인정보 처리방침/고지에 반영하기 전에는 운영 데이터를 넣지 않는다. 한국 리전이면 계정·여행 DB와 사진의 국외 보관은 사라지지만 Google Drive 백업, `SENTRY_DSN`을 넣었을 때의 Sentry처럼 국외에서 처리되는 구성요소는 그대로 남는다.
...
- 앱/API 오류: 서버가 예상 못 한 예외 하나를 Sentry로 올린다. `SENTRY_DSN`이 비어 있으면 아무 데도 보내지 않고 요청 로그만 남는다(기본 꺼짐). 앱은 SDK 없이 거른 한 줄을 `POST /v1/client-errors`로 보내고 서버가 이어받는다. 무엇을 지우는지는 [05-quality-and-operations.md](./05-quality-and-operations.md) 5장
