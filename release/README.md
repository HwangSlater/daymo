...
**지금 상태(2026-09-22):** App Store 는 심사를 요청해 두었고, Google Play 는 개발자 본인 인증 심사 중이다.

| # | 할 일 | 누가 | 왜 |
| --- | --- | --- | --- |
| 1 | ~~Apple Developer Program 가입~~ (완료) | 운영자 | iOS 빌드·TestFlight·제출 모두 필요 |
...
| 6 | Google Play 개발자 계정과 비공개 테스트 | 운영자 | 개인 개발자 계정은 테스터 12명 이상이 14일 동안 비공개 테스트를 해야 프로덕션을 신청할 수 있다. 2026-09-22 본인 인증 심사 중 |
| 7 | ~~실제 기기 스크린샷~~ (2026-09-21 완료) | 운영자 + 개발 | 두 스토어 모두 실제 앱 화면이어야 한다. App Store 10장(1290×2796, 업로드 사본 1284×2778)·Play 8장(1080×1920)과 그래픽 이미지를 만들었다(완성본은 저장소 밖) |
| 8 | ~~앱 버전 1.0.0~~ (완료) | 개발 | `mobile/app.json` 이 1.0.0 이다. 앱 안의 버전 표기는 모두 여기서 읽는다(`mobile/src/appVersion.ts`). 빌드 번호는 EAS가 올린다 |
| 9 | ~~카카오 비즈 앱 심사(이메일 동의항목)~~ (통과) | 카카오 | 2026-09-15 신청 |
| 10 | ~~앱 안에서 처리방침·약관 열기~~ (완료) | 개발 | 로그인 화면과 내 프로필에서 이용약관·개인정보 처리방침을 열 수 있다(`mobile/src/WarmAppShell.tsx`의 `LegalLinks`). |
