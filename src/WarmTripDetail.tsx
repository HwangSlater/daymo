  ...
  return (
    <View>
      <View style={styles.welcomeCard}>
        <View style={styles.welcomeMark}><View style={styles.markCircle} /><View style={styles.markLeaf} /></View>
        <Text style={styles.welcomeEyebrow}>THIS WEEKEND</Text>
        <Text style={styles.welcomeTitle}>맛있는 것만{`\n`}천천히 하기</Text>
        <Text style={styles.welcomeCopy}>버섯전골, 늦은 아침, 그리고 우리 둘만의 숙소.</Text>
      </View>
...
function Preparation({ done, toggle, items, addItem }: { done: string[]; toggle: (item: string) => void; items: typeof packing; addItem: (item: typeof packing[number]) => void }) {
  ...
  return (
    <View>
      <View style={styles.prepareIntro}>
        <Text style={styles.prepareEyebrow}>PACKING, TOGETHER</Text>
        <Text style={styles.prepareTitle}>이번 여행을{`\n`}챙기는 중이에요.</Text>
        <View style={styles.progressRow}><Text style={styles.progressText}>{done.filter((name) => items.some((item) => item.name === name)).length} / {items.length} 완료</Text><Text style={styles.progressNumber}>{percentage}%</Text></View>
      ...
      <SectionLabel label="버섯전골 준비물" action="항목 추가" onPress={() => setAdding(true)} />
...
function Memories() {
  ...
  return (
    <View>
      <View style={styles.memoryWelcome}>
        <Text style={styles.memoryEyebrow}>THE NEXT MEMORY</Text>
        <Text style={styles.memoryTitle}>이번 여행의{`\n`}첫 장면을 기다려요.</Text>
        <Pressable onPress={addPhoto} style={styles.memoryButton}><Text style={styles.memoryButtonText}>사진 남기기</Text></Pressable>
      </View>
...
Object.assign(styles, {
  ...
  memoryButton: { alignSelf: 'flex-start', backgroundColor: '#FF6B5F', borderRadius: 12, paddingVertical: 8, paddingHorizontal: 11, marginTop: 12 }
});
