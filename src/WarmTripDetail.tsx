...
type PackingItem = { id: string; name: string; quantity: string; owner: '함께' | '찬희' | '세인' | '미정'; source: '집에서' | '미리 구매' | '현지 구매'; timing: '미리' | '출발 아침' | '숙소에서' };

const packing: PackingItem[] = [
  { id: 'leaf', name: '깻잎', quantity: '20장', owner: '세인', source: '집에서', timing: '미리' },
  { id: 'onion', name: '양파', quantity: '소스용', owner: '찬희', source: '집에서', timing: '미리' },
  { id: 'stock', name: '코인육수', quantity: '2개', owner: '세인', source: '집에서', timing: '미리' },
  { id: 'beef', name: '소고기', quantity: '250g', owner: '찬희', source: '현지 구매', timing: '숙소에서' },
  { id: 'wasabi', name: '고추냉이', quantity: '따로 챙기기', owner: '미정', source: '집에서', timing: '출발 아침' },
];

export function WarmTripDetail({ done, toggle, onClose }: Props) {
  const [mode, setMode] = useState<ViewMode>('여행');
  const [title, setTitle] = useState('전주 한옥마을');
  const [draftTitle, setDraftTitle] = useState(title);
  const [editingTrip, setEditingTrip] = useState(false);
  const [packingItems, setPackingItems] = useState(packing);
  const [schedule, setSchedule] = useState<ScheduleItem[]>([{ time: '금 · 12:30', title: '온기식탁에서 점심', note: '가산 퍼블릭점', mapUrl: 'https://map.naver.com/p/search/장소' }, { time: '금 · 15:00', title: '달빛한옥 체크인', note: '체크아웃은 일요일 12시', mapUrl: 'https://map.naver.com/p/search/장소' }, { time: '금 · 19:30', title: '함께 저녁 만들기', note: '버섯전골와 주먹밥', mapUrl: '' }]);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator={false}>
        ...
        {mode === '준비' && <Preparation done={done} toggle={toggle} items={packingItems} setItems={setPackingItems} />}
        {mode === '기록' && <Memories />}
      </ScrollView>
  ...
  return (
    <View>
      ...
      <Pressable onPress={() => setSheet('reservation')} style={styles.placeCard}>
        ...
        <View style={styles.placeInfo}><Text style={styles.placeKicker}>DINNER RESERVED</Text><Text style={styles.placeName}>소나기식당</Text><Text style={styles.placeNote}>토요일 디너 · 2명</Text></View>
        <Text style={styles.cardArrow}>→</Text>
      </Pressable>
      <View style={styles.twoCards}>
        <Pressable onPress={() => setSheet('stay')} style={[styles.smallCard, styles.stayCard]}><Text style={styles.smallOverline}>STAY</Text><Text style={styles.smallTitle}>달빛한옥</Text><Text style={styles.smallText}>15:00 check-in</Text></Pressable>
        <Pressable onPress={() => setMode('준비')} style={[styles.smallCard, styles.menuCard]}><Text style={styles.smallOverline}>MAKE</Text><Text style={styles.smallTitle}>버섯전골</Text><Text style={styles.smallText}>재료 5가지</Text></Pressable>
      </View>
...
function Places({ addToSchedule }: { addToSchedule: (item: ScheduleItem) => void }) {
  const [places, setPlaces] = useState<PlaceItem[]>([
    { name: '소나기식당', area: '구로', category: '식당', mapUrl: 'https://map.naver.com/p/search/장소', tags: ['초밥', '디너', '예약'], status: '일정' },
    { name: '구름국수', area: '가산', category: '식당', mapUrl: 'https://map.naver.com/p/search/장소', tags: ['늦은 점심', '웨이팅'], status: '후보' },
    { name: '노을전망대', area: '구로', category: '구경', mapUrl: 'https://map.naver.com/p/search/장소', tags: ['숙소 근처', '비 오는 날'], status: '후보' },
  ]);
...
function Preparation({ done, toggle, items, setItems }: { done: string[]; toggle: (item: string) => void; items: PackingItem[]; setItems: React.Dispatch<React.SetStateAction<PackingItem[]>> }) {
  ...
  const [query, setQuery] = useState('');
  const [filter, setFilter] = useState<'전체' | '내 담당' | '미정' | '완료'>('전체');
  const [activities, setActivities] = useState(['세인이 코인육수를 맡았어요', '찬희가 양파를 준비 완료했어요']);
  const completedCount = done.filter((name) => items.some((item) => item.name === name)).length;
  const percentage = items.length ? Math.round((completedCount / items.length) * 100) : 0;
  const visibleItems = items.filter((item) => {
    const matchesQuery = `${item.name} ${item.quantity} ${item.owner} ${item.source} ${item.timing}`.includes(query.trim());
    const matchesFilter = filter === '전체' || (filter === '내 담당' && item.owner === '찬희') || (filter === '미정' && item.owner === '미정') || (filter === '완료' && done.includes(item.name));
  ...
  const submit = () => {
    ...
    setItems((current) => [...current, ...parsed.map((name, index) => ({ id: `${stamp}-${index}`, name, quantity: quantity.trim(), owner, source, timing }))]);
    setActivities((current) => [`찬희가 준비물 ${parsed.length}개를 추가했어요`, ...current]);
    setNames(''); setQuantity(''); setAdding(false);
  };
  const claim = (item: PackingItem) => { setItems((current) => current.map((value) => value.id === item.id ? { ...value, owner: '찬희' } : value)); setActivities((current) => [`찬희가 ${item.name}을(를) 맡았어요`, ...current]); };
  const complete = (item: PackingItem) => { toggle(item.name); setActivities((current) => [`찬희가 ${item.name}을(를) ${done.includes(item.name) ? '다시 준비 중으로 바꿨어요' : '준비 완료했어요'}`, ...current]); };
  const cycleOwner = (item: PackingItem) => { const order: PackingItem['owner'][] = ['미정', '찬희', '세인', '함께']; const next = order[(order.indexOf(item.owner) + 1) % order.length]; setItems((current) => current.map((value) => value.id === item.id ? { ...value, owner: next } : value)); setActivities((current) => [`${item.name} 담당을 ${next}(으)로 바꿨어요`, ...current]); };

  return <View><View style={styles.packingOverview}><View><Text style={styles.packingOverviewLabel}>함께 준비하는 중</Text><Text style={styles.packingOverviewValue}>{completedCount}<Text style={styles.packingOverviewTotal}> / {items.length} 완료</Text></Text></View><View style={styles.packingPercent}><Text style={styles.packingPercentText}>{percentage}%</Text></View></View><View style={styles.compactTrack}><View style={[styles.progressFill, { width: `${percentage}%` }]} /></View><View style={styles.ownerStats}><View style={styles.ownerStat}><Text style={styles.ownerStatName}>찬희</Text><Text style={styles.ownerStatCount}>{items.filter((item) => item.owner === '찬희' && !done.includes(item.name)).length}개 남음</Text></View><View style={styles.ownerDivider} /><View style={styles.ownerStat}><Text style={styles.ownerStatName}>세인</Text><Text style={styles.ownerStatCount}>{items.filter((item) => item.owner === '세인' && !done.includes(item.name)).length}개 남음</Text></View><View style={styles.ownerDivider} /><View style={styles.ownerStat}><Text style={styles.ownerStatName}>미정</Text><Text style={[styles.ownerStatCount, styles.unassignedText]}>{items.filter((item) => item.owner === '미정').length}개</Text></View></View><View style={styles.prepareSearch}><Text style={styles.placeSearchIcon}>⌕</Text><TextInput value={query} onChangeText={setQuery} placeholder="이름, 담당, 준비 방식 검색" placeholderTextColor="#9AA1AE" style={styles.placeSearchInput} /><Pressable onPress={() => setAdding(true)} style={styles.inlineAdd}><Text style={styles.inlineAddText}>+ 추가</Text></Pressable></View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.packingFilters}>{(['전체', '내 담당', '미정', '완료'] as const).map((item) => <Pressable key={item} onPress={() => setFilter(item)} style={[styles.placeFilter, filter === item && styles.placeFilterActive]}><Text style={[styles.placeFilterText, filter === item && styles.placeFilterTextActive]}>{item}</Text></Pressable>)}</ScrollView><View style={styles.packingList}>{visibleItems.map((item) => { const completed = done.includes(item.name); return <View key={item.id} style={[styles.packingCard, completed && styles.packingCardDone]}><Pressable onPress={() => complete(item)} style={[styles.checkbox, completed && styles.checkboxDone]}><Text style={styles.checkIcon}>{completed ? '✓' : ''}</Text></Pressable><View style={styles.packingBody}><View style={styles.packingTitleRow}><Text style={[styles.checkName, completed && styles.checkNameDone]}>{item.name}</Text>{item.quantity ? <Text style={styles.packingQuantity}>{item.quantity}</Text> : null}</View><View style={styles.packingTags}><View style={styles.packingTag}><Text style={styles.packingTagText}># {item.source}</Text></View><View style={styles.packingTag}><Text style={styles.packingTagText}># {item.timing}</Text></View></View>{item.owner === '미정' && !completed ? <Pressable onPress={() => claim(item)} style={styles.claimButton}><Text style={styles.claimButtonText}>내가 챙길게</Text></Pressable> : null}</View><Pressable onPress={() => cycleOwner(item)} style={[styles.ownerBadge, item.owner === '미정' && styles.ownerBadgeEmpty]}><Text style={[styles.ownerBadgeText, item.owner === '미정' && styles.ownerBadgeTextEmpty]}>{item.owner}</Text><Text style={styles.ownerChange}>변경</Text></Pressable></View>; })}</View>{visibleItems.length === 0 && <View style={styles.emptyPacking}><Text style={styles.emptyPackingText}>조건에 맞는 준비물이 없어요.</Text></View>}<View style={styles.activityBox}><View style={styles.activityHead}><Text style={styles.activityTitle}>최근 함께한 일</Text><Text style={styles.activityLive}>● LIVE</Text></View>{activities.slice(0, 3).map((activity, index) => <View key={`${activity}-${index}`} style={styles.activityRow}><View style={styles.activityDot} /><Text style={styles.activityText}>{activity}</Text>{index === 0 && <Text style={styles.activityTime}>방금</Text>}</View>)}</View><DetailSheet visible={adding} title="준비물 함께 추가" subtitle="여러 개를 한 번에 적고 담당과 준비 방식을 정하세요" submit={names.split(/[\n,]/).filter((name) => name.trim()).length ? `${names.split(/[\n,]/).filter((name) => name.trim()).length}개 추가` : '준비물을 입력해 주세요'} onClose={() => setAdding(false)} onSubmit={submit}><View style={styles.quickAdd}><Text style={styles.quickAddLabel}>자주 챙기는 것</Text><View style={styles.quickRow}>{['충전기', '세면도구', '갈아입을 옷', '우산'].map((item) => <Pressable key={item} onPress={() => setNames((value) => value ? `${value}, ${item}` : item)} style={styles.quickChip}><Text style={styles.quickChipText}>+ {item}</Text></Pressable>)}</View></View><DetailField label="준비물 · 여러 개 가능" value={names} onChangeText={setNames} placeholder={'충전기, 안경, 갈아입을 옷'} multiline /><OptionField label="누가 챙길까요?" options={['미정', '찬희', '세인', '함께']} value={owner} onChange={(value) => setOwner(value as PackingItem['owner'])} /><OptionField label="어디서 준비할까요?" options={['집에서', '미리 구매', '현지 구매']} value={source} onChange={(value) => setSource(value as PackingItem['source'])} /><OptionField label="언제 확인할까요?" options={['미리', '출발 아침', '숙소에서']} value={timing} onChange={(value) => setTiming(value as PackingItem['timing'])} /><DetailField label="수량 · 선택" value={quantity} onChangeText={setQuantity} placeholder="예: 각 2개, 250g" /></DetailSheet></View>;
}

function Memories() {
  const [notes, setNotes] = useState([{ author: '세인 · 오늘 10:42', body: '육수 재료는 내가 미리 1.5배로 만들어갈게!' }, { author: '찬희 · 어제 22:15', body: '소나기식당은 일요일 13:30으로 생각하고 있어요.' }]);
...
const styles = StyleSheet.create({
  ...
  packingOverview: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingTop: 2 }, packingOverviewLabel: { color: '#7168A5', fontSize: 10, fontWeight: '900' }, packingOverviewValue: { color: '#17233D', fontSize: 27, fontWeight: '900', marginTop: 4 }, packingOverviewTotal: { color: '#858C98', fontSize: 11, fontWeight: '800' }, packingPercent: { width: 52, height: 52, borderRadius: 18, backgroundColor: '#8B7CF6', alignItems: 'center', justifyContent: 'center' }, packingPercentText: { color: '#FFFFFF', fontSize: 13, fontWeight: '900' },
  ownerStats: { minHeight: 61, borderRadius: 17, backgroundColor: '#FFFFFF', flexDirection: 'row', alignItems: 'center', marginTop: 13, marginBottom: 14, paddingHorizontal: 8 }, ownerStat: { flex: 1, alignItems: 'center' }, ownerStatName: { color: '#17233D', fontSize: 10, fontWeight: '900' }, ownerStatCount: { color: '#89909C', fontSize: 8, fontWeight: '800', marginTop: 4 }, unassignedText: { color: '#D25A50' }, ownerDivider: { width: 1, height: 26, backgroundColor: '#ECEAE5' },
  inlineAdd: { height: 31, borderRadius: 10, backgroundColor: '#17233D', paddingHorizontal: 10, alignItems: 'center', justifyContent: 'center' }, inlineAddText: { color: '#FFFFFF', fontSize: 9, fontWeight: '900' }, packingFilters: { gap: 6, paddingBottom: 13 },
  packingList: { gap: 9 }, packingCard: { minHeight: 83, borderRadius: 19, backgroundColor: '#FFFFFF', paddingHorizontal: 13, paddingVertical: 12, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#ECEAE5' }, packingCardDone: { backgroundColor: '#F1F0EC', opacity: .76 }, packingBody: { flex: 1 }, packingTitleRow: { flexDirection: 'row', alignItems: 'center', gap: 7 }, packingQuantity: { color: '#858D99', fontSize: 9, fontWeight: '800' },
  claimButton: { alignSelf: 'flex-start', height: 25, borderRadius: 9, backgroundColor: '#FFF0ED', paddingHorizontal: 9, alignItems: 'center', justifyContent: 'center', marginTop: 7 }, claimButtonText: { color: '#D25A50', fontSize: 8, fontWeight: '900' }, ownerBadge: { minWidth: 49, height: 46, borderRadius: 14, backgroundColor: '#E9E5FF', alignItems: 'center', justifyContent: 'center', marginLeft: 8, paddingHorizontal: 6 }, ownerBadgeEmpty: { backgroundColor: '#FFF0ED' }, ownerBadgeText: { color: '#6556D8', fontSize: 10, fontWeight: '900' }, ownerBadgeTextEmpty: { color: '#D25A50' }, ownerChange: { color: '#9A94B8', fontSize: 6, fontWeight: '800', marginTop: 3 },
  emptyPacking: { height: 90, alignItems: 'center', justifyContent: 'center' }, emptyPackingText: { color: '#9299A4', fontSize: 11 }, activityBox: { borderRadius: 19, backgroundColor: '#17233D', padding: 15, marginTop: 18 }, activityHead: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }, activityTitle: { color: '#FFFFFF', fontSize: 11, fontWeight: '900' }, activityLive: { color: '#65D8CA', fontSize: 7, letterSpacing: .6, fontWeight: '900' }, activityRow: { minHeight: 32, flexDirection: 'row', alignItems: 'center' }, activityDot: { width: 5, height: 5, borderRadius: 3, backgroundColor: '#8B7CF6', marginRight: 8 }, activityText: { flex: 1, color: '#B6C0D0', fontSize: 9 }, activityTime: { color: '#65D8CA', fontSize: 7, fontWeight: '800' },
});
