...
import { Modal, PanResponder, Platform, Pressable, SafeAreaView, ScrollView, Share, StyleSheet, Text, TextInput, View } from 'react-native';
...
const trips: Trip[] = [
  { name: '전주 한옥마을', date: '8월 21일 — 23일', note: '느긋한 숙소와 버섯전골', color: '#FF6B5F', mark: '08', region: '서울', start: '2026-08-21', end: '2026-08-23' },
  { name: '강릉 안목', date: '8월 1일 — 2일', note: '생새우 파티와 치킨', color: '#8B7CF6', mark: '08', region: '경기', start: '2026-08-01', end: '2026-08-02' },
...
function Home({ open, goTrips }: { open: () => void; goTrips: () => void }) {
  return <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.page}>
    ...
    <Pressable onPress={open} style={s.nextCard}><TripArt color="#19B6A3" date="08 · 21" /><View style={s.nextText}><Text style={s.nextTag}>D−12 · PLANNING</Text><Text style={s.nextTitle}>전주 한옥마을</Text><Text style={s.nextDate}>8월 21일 — 23일 · 2박 3일</Text><Text style={s.nextNote}>느긋한 숙소와 버섯전골</Text></View></Pressable>
...
function KoreaTripMap({ trips, results, selected, onSelect, open }: { trips: Trip[]; results: Trip[]; selected: string | null; onSelect: (region: string) => void; open: () => void }) {
  ...
  const clampCenter = (point: { x: number; y: number }, level: number) => { const halfWidth = 150 / level; const halfHeight = 210 / level; return { x: Math.max(halfWidth, Math.min(300 - halfWidth, point.x)), y: Math.max(halfHeight, Math.min(420 - halfHeight, point.y)) }; };
  const changeZoom = (amount: number, focus = { x: size.width / 2, y: size.height / 2 }) => { const nextZoom = Math.max(1, Math.min(MAP_MAX_ZOOM, Math.round((zoom + amount) * 2) / 2)); if (nextZoom === zoom) return; const mapX = boxX + (focus.x - mapOffsetX) / mapScale; const mapY = boxY + (focus.y - mapOffsetY) / mapScale; const nextWidth = 300 / nextZoom; const nextHeight = 420 / nextZoom; const nextScale = Math.min(size.width / nextWidth, size.height / nextHeight); const nextOffsetX = (size.width - nextWidth * nextScale) / 2; const nextOffsetY = (size.height - nextHeight * nextScale) / 2; setCenter(clampCenter({ x: mapX - (focus.x - nextOffsetX) / nextScale + nextWidth / 2, y: mapY - (focus.y - nextOffsetY) / nextScale + nextHeight / 2 }, nextZoom)); setZoom(nextZoom); };
  const panResponder = useMemo(() => PanResponder.create({ onMoveShouldSetPanResponder: (_, gesture) => zoom > 1 && Math.abs(gesture.dx) + Math.abs(gesture.dy) > 4, onPanResponderGrant: () => { dragStart.current = centerRef.current; }, onPanResponderMove: (_, gesture) => setCenter(clampCenter({ x: dragStart.current.x - gesture.dx / mapScale, y: dragStart.current.y - gesture.dy / mapScale }, zoom)) }), [mapScale, zoom]);
  const webWheel = Platform.OS === 'web' ? { onWheel: (event: { preventDefault?: () => void; nativeEvent?: { deltaY?: number; locationX?: number; locationY?: number }; deltaY?: number }) => { event.preventDefault?.(); const now = Date.now(); if (now - lastWheel.current < 120) return; lastWheel.current = now; const native = event.nativeEvent; changeZoom((native?.deltaY ?? event.deltaY ?? 0) < 0 ? .5 : -.5, { x: native?.locationX ?? size.width / 2, y: native?.locationY ?? size.height / 2 }); } } : {};
  const cityPath: string | null = zoom >= 2 ? require('./koreaCityPath').koreaCityPath : null;
  return <View {...webWheel as any} style={(s as any).mapOnly} onLayout={(event) => setSize(event.nativeEvent.layout)}><Svg width="100%" height="100%" viewBox={viewBox} preserveAspectRatio="xMidYMid meet"><Path d={koreaLandPath} fill="#DDF4EF" stroke="none" /><Path d={koreaOutlinePath} fill="none" stroke="#159D8D" strokeWidth={2.2 / zoom} strokeLinejoin="round" strokeLinecap="round" /><Path d={koreaAdminPath} fill="none" stroke="#4DA99E" strokeWidth={0.75 / zoom} strokeLinejoin="round" strokeLinecap="round" />{zoom >= 2 && <Path d={cityPath!} fill="none" stroke="#8CCFC7" strokeWidth={0.38 / zoom} strokeLinejoin="round" strokeLinecap="round" />}</Svg><View {...panResponder.panHandlers} style={(s as any).mapDragLayer} />{regionPins.map((pin) => { const count = trips.filter((trip) => trip.region === pin.name).length; const active = selected === pin.name; const left = mapOffsetX + (pin.x - boxX) * mapScale; const top = mapOffsetY + (pin.y - boxY) * mapScale; const visible = zoom >= pin.minZoom && left > -24 && left < size.width + 24 && top > -16 && top < size.height + 16; return visible ? <Pressable key={pin.name} onPress={() => { if (zoom > 1) setCenter(clampCenter({ x: pin.x, y: pin.y }, zoom)); onSelect(pin.name); }} style={[(s as any).mapPin, { left, top }, count > 0 && (s as any).mapPinVisited, active && (s as any).mapPinActive]}><Text numberOfLines={1} style={[(s as any).mapPinText, (count > 0 || active) && (s as any).mapPinTextVisited]}>{pin.name}</Text>{count > 0 && <View style={(s as any).pinCount}><Text style={(s as any).pinCountText}>{count}</Text></View>}</Pressable> : null; })}{selected && <View style={(s as any).mapTray}><View style={(s as any).mapTrayHandle} /><View style={(s as any).mapTrayHead}><View><Text style={(s as any).mapTrayTitle}>{selected} 여행</Text><Text style={(s as any).mapTrayCount}>{results.length ? `${results.length}개의 여행` : '아직 등록된 여행이 없어요'}</Text></View><Pressable onPress={() => onSelect(selected)} style={(s as any).mapTrayClose}><Text style={(s as any).mapTrayCloseText}>×</Text></Pressable></View>{results.length ? <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={(s as any).mapTrayList}>{results.map((trip, index) => <Pressable key={`${trip.name}-${index}`} onPress={open} style={(s as any).mapTrayCard}><View style={[(s as any).mapTrayMark, { backgroundColor: trip.color }]}><Text style={(s as any).mapTrayMarkText}>{trip.mark}</Text></View><View style={(s as any).mapTrayCopy}><Text numberOfLines={1} style={(s as any).mapTrayName}>{trip.name}</Text><Text numberOfLines={1} style={(s as any).mapTrayDate}>{trip.date}</Text></View><Text style={(s as any).mapTrayArrow}>›</Text></Pressable>)}</ScrollView> : <Text style={(s as any).mapTrayEmpty}>다른 지역을 눌러 여행을 찾아보세요.</Text>}</View>}<View style={[(s as any).zoomControls, selected && (s as any).zoomControlsRaised]}><Pressable disabled={zoom <= 1} onPress={() => changeZoom(-.5)} style={[(s as any).zoomButton, zoom <= 1 && (s as any).zoomButtonDisabled]}><Text style={(s as any).zoomText}>−</Text></Pressable><View style={(s as any).zoomDivider} /><Pressable disabled={zoom >= MAP_MAX_ZOOM} onPress={() => changeZoom(.5)} style={[(s as any).zoomButton, zoom >= MAP_MAX_ZOOM && (s as any).zoomButtonDisabled]}><Text style={(s as any).zoomText}>＋</Text></Pressable></View></View>;
}
...
function Search({ open }: { open: () => void }) {
  ...
  const results = useMemo(() => [{ title: '소나기식당', type: '장소', trip: '전주 한옥마을' }, { title: '버섯전골', type: '함께 해먹기', trip: '전주 한옥마을' }, { title: '구름국수', type: '장소', trip: '강릉 안목' }, { title: '충전기', type: '준비물', trip: '진주' }].filter((item) => item.title.includes(query)), [query]);
...
Object.assign(s, {
  ...
  mapTrayHead: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, marginBottom: 9 },
  mapTrayTitle: { color: '#17233D', fontSize: 15, fontWeight: '900' }, mapTrayCount: { color: '#7D8987', fontSize: 9, marginTop: 2 },
  mapTrayClose: { width: 28, height: 28, borderRadius: 14, backgroundColor: '#F0F3F2', alignItems: 'center', justifyContent: 'center' }, mapTrayCloseText: { color: '#66716F', fontSize: 20, lineHeight: 21 },
  mapTrayList: { paddingHorizontal: 12, gap: 8 }, mapTrayCard: { width: 244, height: 60, borderRadius: 16, padding: 8, backgroundColor: '#F7F8F6', borderWidth: 1, borderColor: '#E7EBE8', flexDirection: 'row', alignItems: 'center' },
  mapTrayMark: { width: 43, height: 43, borderRadius: 12, alignItems: 'center', justifyContent: 'center' }, mapTrayMarkText: { color: '#FFFFFF', fontSize: 11, fontWeight: '900' },
  mapTrayCopy: { flex: 1, paddingHorizontal: 10 }, mapTrayName: { color: '#17233D', fontSize: 12, fontWeight: '900' }, mapTrayDate: { color: '#8A918F', fontSize: 9, marginTop: 4 }, mapTrayArrow: { color: '#159D8D', fontSize: 20 }, mapTrayEmpty: { color: '#8A918F', fontSize: 10, paddingHorizontal: 16, paddingTop: 8 },
