...
| 우리 | 여러 공간 전환 · 이름으로 멤버 추가와 권한(관리자·편집 가능·보기만) 지정 · 공간 프로필과 관계 설정 · 여행 통계 · 앱 색상과 화면 모드 · 도움말 · 오픈소스 라이선스 · 앱 버전 표시 · 여행 기록 내보내기 · 로그아웃과 「이 기기 데이터 모두 지우기」 |
...
- **서버와 인증이 없다.** 여행 계획과 준비물 완료 상태, 공간과 멤버는 기기에 저장하지만, 여러 사람 사이의 동기화와 실제 계정 인증은 아직 없다. 적은 것은 이 기기 밖으로 나가지 않는다.
...
- **서버가 여행까지 왔고 앱이 아직 안 붙었다.** 가입·로그인·세션과 공간·여행 API가 열렸지만 장소·준비물·비용·기록은 아직 API가 없다. 소셜 로그인도, 실제 메일 발송도 아직이다. 앱은 여전히 서버에 접속하지 않는다.
