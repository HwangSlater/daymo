...
> 현재는 **UI 프로토타입**이다. 서버와 인증은 아직 붙지 않았고 기록은 이 기기에만 남는다. 처음 담겨 있는 여행 넷은 가명 시드고, 새로 만든 여행은 빈 채로 시작한다. [아직 안 된 것](#아직-안-된-것)을 먼저 보라.
...
npm test             # node --test, 74개
```

타입 검사·lint·테스트 세 가지는 main 푸시와 모든 pull request에서 GitHub Actions가 그대로 돌린다(`.github/workflows/ci.yml`). 테스트는 지출과 정산 셈, 시각 입력, 공간과 멤버, 파일 이름, 네이버 장소 조회를 덮는다.
...
- **여행 상세 기록은 아직 기기에만 있다.** 가입·로그인·세션, 공간과 멤버 보기, 여행 목록·만들기·수정, 계정 삭제는 서버에 붙었다. 일정·장소·준비물·요리·비용·기록은 API가 없어서 이 기기 밖으로 나가지 않고, 여러 사람 사이에 동기화되지 않는다.
- **소셜 로그인은 키를 넣어야 켜진다.** 카카오·네이버·Google·Apple 로그인 흐름은 서버와 앱에 모두 있지만, 아직 어느 provider 콘솔에도 앱을 등록하지 않았다. 서버에 키가 들어간 provider만 앱에 버튼이 보이므로 지금은 아무 버튼도 나오지 않는다(등록 방법은 `docs/development/11-owner-setup-guide.md` 7.1). 버튼 심볼은 각 사 공식 에셋으로 아직 바꾸지 않았고, 소셜로만 가입한 계정은 비밀번호 재확인이 필요한 계정 삭제를 아직 할 수 없다.
- **멤버를 초대할 수 없다.** 초대 링크·권한 변경·내보내기 API가 아직 없다. 서버에 없는 사람이 담당과 정산에 들어가면 다른 기기에서 보이지 않아서, 이름만 적어 넣는 멤버 추가는 막았다. 「우리」 탭의 멤버는 지금 읽기 전용이다.
- **권한은 서버가 지키지만 앱이 아직 다 따르지는 않는다.** 공간 정보는 관리자에게만 칸이 열린다. 여행 하위 화면은 아직 서버에 붙지 않아서 「보기만」인 사람도 기기 안에서는 고칠 수 있다.
- **알림이 없다.** 앱에 알림 코드가 없어서, state만 뒤집던 가짜 알림 토글은 지웠다.
- **내 이름을 서버에서 바꿀 수 없다.** 이름은 기기에만 바뀌고 이메일은 바꿀 수 없다. 다른 멤버가 있는 공간의 관리자는 관리자를 넘기는 기능이 없어 계정을 삭제할 수 없다.
...
장소 추가에서 네이버 지도 공유 링크를 붙여넣으면 앱이 장소명과 주소를 자동으로 채운다. 공유 문구에 정보가 들어 있으면 기기에서 바로 읽고, 짧은 링크만 있으면 `EXPO_PUBLIC_DAYMO_PLACE_RESOLVER_URL`의 서버 함수가 링크를 확인한다. Vercel로 `mobile`을 배포하면 함수 주소는 `https://<도메인>/api/naver-place`다.
