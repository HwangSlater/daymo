...
<table>
  ...
  <tr>
    ...
    <td width="25%" valign="top">공간과 멤버를 관리한다. 현재 공간을 바꾸고, 멤버를 이름으로 추가하고, 멤버별 권한(관리자·편집 가능·보기만)을 정한다. 서버가 없어 권한은 아직 이름표라고 화면에도 적어 뒀다. 아래로는 함께 쌓은 여행 통계와 앱 설정이 이어진다.</td>
  </tr>
</table>
...
backend/    동기화 API. 스키마까지 왔고 API는 아직 없다
...
- **초대 링크를 발급할 수 없다.** 서버가 없어서다. 대신 멤버를 이름으로 직접 적어 넣는다.
- **멤버 권한은 아직 이름표다.** 관리자·편집 가능·보기만을 고를 수는 있지만, 서버가 없으면 「보기만」인 사람이 고치는 것을 막을 수 없다. 지금은 누가 무엇을 맡는지 적어 두는 용도이고, 화면에도 그렇게 적어 뒀다.
- **알림이 없다.** 앱에 알림 코드가 없어서, state만 뒤집던 가짜 알림 토글은 지웠다.
- **서버에 계정만 있고 여행이 없다.** `backend/`에 이메일 가입·로그인·세션 API가 열렸지만 여행·장소·비용은 아직 API가 없다. 소셜 로그인도, 실제 메일 발송도 아직이다. 앱은 여전히 서버에 접속하지 않는다.
