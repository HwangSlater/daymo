...
| 로그인 | 이메일 로그인·회원가입(필수/선택 약관 동의 구분) · 이메일 확인 · 첫 여행 공간 만들기 · 삭제를 요청해 둔 계정의 삭제 취소 화면 |
...
- **소셜 로그인이 없다.** 카카오·네이버·Google·Apple 로그인은 서버 쪽이 없어서 버튼을 숨겨 뒀다(`SOCIAL_LOGIN_READY`). 눌러도 되지 않는 버튼은 스토어 심사에서 반려 사유다.
