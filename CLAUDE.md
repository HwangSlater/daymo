...
- UI 문구는 `docs/development/13-copy-glossary.md` 를 따른다. 새 용어는 사전에 먼저 적는다.
- **`copyGlossary` 검사는 적어 둔 금지어만 잡는다. 통과했다고 사전을 지킨 것이 아니다.** 새 글자를 쓸 때는 검사를 믿지 말고 사전을 먼저 본다(2026-09-23 검토에서 「지우기」·「~하세요」·「내보내기」 같은 것이 28개 파일에 새어 있었다. 그때 검사 패턴을 16개 넓혔다).
...
- **값을 직접 적지 않는다.** 모서리·글자 크기·색·여백은 토큰을 쓴다. 토큰에 없는 값이 필요하면 토큰을 늘리고 문서에 적는다 — 스타일 옆에 숫자를 적어 두면 다음 사람이 그것을 따라 적는다(2026-09-23에 칩이 14종, 토스트가 3벌, 빈 화면이 5모양이었다).
- **부품으로 옮기면 원래 스타일과 JSX 를 지운다.** 남겨 두면 어느 쪽이 진짜인지 알 수 없고 줄만 는다.
...
2026-09-23 앱 전체 검토에서 P0 여덟 개가 나왔다. 그중 다섯은 아래를 돌렸으면 애초에 없었을 것들이고, 대부분 새 기능이 아니라 **고치다가** 생겼다. 새로 만들 때는 아래를 다 돌고, 고칠 때는 **내가 건드린 면에 해당하는 것만** 돈다.

- **고치기 전에 그것을 쓰는 다른 자리를 먼저 찾는다**(`grep` 으로 호출처). 사진 받는 길을 고쳤더니 카드에 넣을 사진 목록이 비었다(2026-09-23). 화면 하나를 보고 고쳤는데 다른 화면이 같은 것을 쓰고 있었다.
- 무엇을 건드렸는지로 고를 것: **문구를 건드렸으면** 사전 / **저장·서버로 가는 값을 건드렸으면** 데이터 절 / **화면을 건드렸으면** 아래 네 가지 / **목록·사진을 건드렸으면** 규모.

- **네 가지로 한 번씩 열어 본다: 빈 값 · 아주 긴 값 · 실패(오프라인) · 보기 전용 권한.** 죽은 「관리」 버튼, 새 공간의 달력 `NaN`, 서버 실패 때 뜨던 가상 데이터, 보기 전용 멤버에게 보이던 「새 여행」이 전부 여기서 걸렸을 것들이다.
- **목록·사진을 다루면 100개·1,000개일 때를 한 번 생각한다.** 못 재겠으면 「측정 필요」라고 적어 둔다. 여행을 열 때 남이 올린 사진을 전부 받아 영구 보관하던 것이 그렇게 자랐다.
- **사용자가 한 일이 실패하면 반드시 알린다.** `catch(() => undefined)` 는 개인 편의 값(테마·한 번 본 안내)에만 쓴다.
- **지우는 것에는 되돌리기나 확인을 둔다.** 되돌릴 수 없으면 확인창에 무엇이 사라지는지 적는다.
- **기다리게 하면 무엇을 하는 중인지 보여 준다.** 글자 한 줄로 멈춘 듯 두지 않는다(`CardDeveloping.tsx` 가 본보기다).
- **손가락이 닿는 것은 44pt.** 그보다 작게 그렸으면 `누름여유` 로 채운다. 글자를 키운 기기에서 한 번 본다.
- 「된다」는 기기에서 본 뒤에 말한다(위 「확인 방식」).
...
- **새 저장 열쇠(`daymo.*`)를 만들면 로그아웃·계정 전환·「이 기기 데이터 모두 삭제」에서 어떻게 되는지 같은 자리에서 정한다.** 열쇠가 아홉 개가 되는 동안 로그아웃은 하나만 지우고 있어서, 한 기기를 번갈아 쓰면 앞사람의 아직 못 올린 기록이 뒷사람 계정으로 올라갔다(2026-09-23). 지금은 `daymo.device-owner.v1` 로 막고, 열쇠 목록은 `docs/development/00-repo-guide.md` 에 있다.
- **저장 형식이나 정책을 바꾸면 이미 나간 판을 쓰는 사람의 데이터가 어떻게 되는지 함께 정한다.** 추억 카드를 서버에서 기기로 옮기면서 1.0.0 사용자의 카드가 화면에서 사라졌다(2026-09-23. 들여오기를 나중에 따로 넣었다).
- **날짜는 `YYYY-MM-DD` 키로 들고 다니고 보이는 글자는 그릴 때 만든다. 돈은 통화와 소수 자릿수를 함께 본다.** 지출·사진이 「3일(금)」 같은 표시 이름표를 들고 있어서, 여행 기간을 옮기면 서버의 날짜가 지워졌다(2026-09-23).
...
- **화면 파일이 커지면 계산부터 `.ts` 로 뺀다.** `WarmTripDetail.tsx`·`WarmAppShell.tsx` 가 앱 코드의 43% 가 되면서 파일이 변경 단위가 됐다 — 에이전트에게 파일 단위로 나눠 맡기는 것이 이 둘에서는 안 된다. 계산을 빼면 시험도 붙는다.
